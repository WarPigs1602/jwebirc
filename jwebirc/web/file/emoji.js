...
window.EMOJIES = EMOJIES;
		...
		for (var i in this) {
			if (Object.prototype.hasOwnProperty.call(this, i)) {
				Object.defineProperty(this, i, {
					configurable: false,
					writable: false,
					enumerable: unlisted.indexOf(i) === -1,
					value: this[i]
				});
			}
		}
	...
	function EmojiSubCategory(name, emojis) {
		...
		for (var emoji in emojis) {
			if (Object.prototype.hasOwnProperty.call(emojis, emoji)) {
				var code = emoji;
				var ename = emojis[emoji];
				for (var n in EMOJIES._parser) {
					if (Object.prototype.hasOwnProperty.call(EMOJIES._parser, n)) {
						code = code.replace(new RegExp(n, 'g'), EMOJIES._parser[n][0]);
						ename = ename.replace(new RegExp(n, 'g'), EMOJIES._parser[n][1]);
					}
				}
				this[code] = new Emoji(code, ename);
			}
		}
		for (var j in this) {
			if (Object.prototype.hasOwnProperty.call(this, j)) {
				Object.defineProperty(this, j, {
					configurable: false,
					writable: false,
					enumerable: unlisted.indexOf(j) === -1,
					value: this[j]
				});
			}
		}
	...
	function Emoji(code, name) {
		this.name = name;
		this.code = code;
		this.skintone = name.indexOf('skin tone') !== -1;
		this.skintonetype = -1;
		var tones = ['1F3FB', '1F3FC', '1F3FD', '1F3FE', '1F3FF'];
		for (var t = 0; t < tones.length; t++) {
			if (code.indexOf(tones[t]) !== -1) this.skintonetype = t;
		}
		this.combined = code.indexOf('200D') !== -1;
		this.unicode = String.fromCodePoint.apply(String, code.split(' ').map(function(v) {
			return parseInt(v, 16);
		}));
		this.male = false;
		var males = ['men', 'man', 'boy'];
		for (var m = 0; m < males.length; m++) {
			if (name.indexOf(males[m]) !== -1) this.male = true;
		}
		this.female = false;
		var females = ['female', 'woman', 'women', 'girl'];
		for (var f = 0; f < females.length; f++) {
			if (name.indexOf(females[f]) !== -1) this.female = true;
		}
	...
	function Emojilist() {
		...
		if (EMOJIES && this instanceof Emojilist) {
			for (var cat in EMOJIES) {
				if (Object.prototype.hasOwnProperty.call(EMOJIES, cat) && cat !== '_parser') {
		...
		for (var c in this) {
			if (Object.prototype.hasOwnProperty.call(this, c)) {
				Object.defineProperty(this, c, {
					configurable: false,
					writable: false,
					enumerable: unlisted.indexOf(c) === -1,
					value: this[c]
				});
			}
		}
	...
	Emojilist.prototype.loadRecent = function() {};
	...
	!function() {
		...
		function getRadioValue(wrapper, selector) {
			var checked = wrapper.querySelector(selector + ' input[type=radio]:checked');
			return checked ? checked.value : null;
		}

		function skinToneCompatible(el, val) {
			if (!val || val === '-1') {
				return !el.hasAttribute('emoji-skintone');
			}
			return el.getAttribute('emoji-skintone') === val;
		}

		function familyTypeCompatible(el, val) {
			if (!val || val === '👨‍👩‍👧‍👦') {
				return true;
			}
			var target = val.codePointAt(0).toString(16).toUpperCase();
			return el.getAttribute('emoji-code').split(' ').indexOf(target) !== -1;
		}

		function genderCompatible(el, val) {
			if (!val || val === '❎') {
				return true;
			}
			var target = val === '♂' ? '2642' : '2640';
			return el.getAttribute('emoji-code').split(' ').indexOf(target) !== -1;
		}

		function applyFilters(wrapper) {
			var skinVal = getRadioValue(wrapper, '.emoji-style-picker');
			var typeVal = getRadioValue(wrapper, '.emoji-type-picker');
			var genderVal = getRadioValue(wrapper, '.emoji-gender-picker');
			var SMILEYS = 'Smileys & People';
			qsAll(wrapper, '.emoji-list .tab-pane').forEach(function(pane) {
				var isSmileys = pane.getAttribute('data-emoji-category') === SMILEYS;
				qsAll(pane, '.emoji').forEach(function(el) {
					if (!isSmileys) {
						el.style.display = '';
						return;
					}
					var visible = skinToneCompatible(el, skinVal) &&
						familyTypeCompatible(el, typeVal) &&
						genderCompatible(el, genderVal);
		...
		function initButtonGroup(group) {
			qsAll(group, 'input[type=radio]').forEach(function(radio) {
				radio.addEventListener('change', function() {
					qsAll(group, 'label').forEach(function(label) {
						label.classList.remove('active');
					});
					var label = radio.closest ? radio.closest('label') : radio.parentNode;
					if (label) label.classList.add('active');
					var wrapper = group.closest('.emoji-wrapper');
					if (wrapper) applyFilters(wrapper);
				});
			});
		}
		...
		function createButtonGroup(options) {
			...
			group.className = 'btn-group btn-group-toggle ' + options.className;
			var name = randomId('radio-');
			options.items.forEach(function(item, i) {
				var label = document.createElement('label');
				label.className = 'btn btn-light btn-sm border-secondary' + (i ? '' : ' active');
		...
		function buildFilterBar(wrapper) {
			...
			filters.className = 'emoji-filters';
			filters.appendChild(createButtonGroup({
				className: 'emoji-style-picker',
				items: [
					{ value: '-1', label: '❎' },
					{ value: '🏻', label: '🏻' },
					{ value: '🏼', label: '🏼' },
		...
		function createPopover(trigger, content, options) {
			options = options || {};
			var placement = options.placement || 'top';
			var popover = document.createElement('div');
			popover.className = 'popover custom-emoji-popover';
			popover.setAttribute('role', 'tooltip');
			popover.style.cssText = 'position:fixed;display:none;z-index:1080;width:min(50vw,420px);max-width:calc(100vw - 16px);height:min(50vh,360px);max-height:calc(100vh - 16px);background:var(--surface-elevated,#fff);color:var(--text-primary,#212529);border:1px solid var(--border-color,#ccc);border-radius:8px;box-shadow:0 8px 24px rgba(0,0,0,.18);overflow:hidden;';
			var body = document.createElement('div');
			body.className = 'popover-body';
			body.style.cssText = 'position:relative;width:100%;height:100%;overflow:auto;padding:0;';
			...
			document.addEventListener('click', function(e) {
				if (!visible) return;
				if (popover.contains(e.target) || trigger === e.target || trigger.contains(e.target)) return;
				hide();
			});
			window.addEventListener('resize', function() {
				if (visible) position();
			});
			return { show: show, hide: hide, toggle: toggle, el: popover };
		}
		...
		Emojilist.prototype.attach = function(conf) {
			...
			wrapper.innerHTML = [
				'<ul class="nav nav-tabs emoji-menu" role="tablist"></ul>',
				'<div class="tab-content emoji-list"></div>'
			].join('');
			var menu = wrapper.querySelector('.emoji-menu');
			var list = wrapper.querySelector('.emoji-list');
			var filters = buildFilterBar(wrapper);
			// Categories (menu) → filters → emoji list
			if (list.nextSibling) {
			...
			var node = conf.node;
			var clickaction = typeof conf.insert === 'function' ? conf.insert : function() {};
			var SMILEYS_CATEGORY = 'Smileys & People';

			function setFiltersVisible(visible) {
				filters.style.display = visible ? '' : 'none';
				wrapper.classList.toggle('emoji-filters-visible', !!visible);
			}

			function categoryI18nKey(name) {
				return {
					'Recent': 'emoji.category.recent',
					'Smileys & People': 'emoji.category.smileys',
					'Animals & Nature': 'emoji.category.nature',
					'Food & Drink': 'emoji.category.food',
					'Travel & Places': 'emoji.category.travel',
			...
			function categoryLabel(name) {
				var key = categoryI18nKey(name);
				var fallback = name;
				if (key && typeof window.jwebircTranslate === 'function') {
					var translated = window.jwebircTranslate(key);
					if (translated && translated !== key) return translated;
				}
				return fallback;
			}

			function addNav(id, title, categoryName) {
				var displayTitle = title;
				if (typeof conf.pageTitle === 'function') displayTitle = conf.pageTitle(title);
				var label = categoryLabel(categoryName || title);
				var i18nKey = categoryI18nKey(categoryName || title);
				var li = document.createElement('li');
				li.className = 'nav-item';
				var a = document.createElement('a');
				a.className = menu.childNodes.length ? 'nav-link' : 'nav-link active';
				a.id = randomId(id + '-');
				var paneId = randomId('emoji-tab-');
				a.href = '#' + paneId;
				a.setAttribute('role', 'tab');
				a.setAttribute('aria-controls', paneId);
				a.setAttribute('aria-selected', String(!menu.childNodes.length));
				a.setAttribute('data-emoji-category', categoryName || title);
				a.setAttribute('title', label);
				a.setAttribute('aria-label', label);
				if (i18nKey) {
					a.setAttribute('data-i18n-title', i18nKey);
				...
				a.addEventListener('click', function(e) {
					e.preventDefault();
					e.stopPropagation();
					qsAll(wrapper, '.nav-link').forEach(function(link) {
						link.classList.remove('active');
						link.setAttribute('aria-selected', 'false');
					});
					a.classList.add('active');
					a.setAttribute('aria-selected', 'true');
					qsAll(wrapper, '.tab-pane').forEach(function(pane) {
						pane.classList.remove('active', 'show');
					});
					var target = document.getElementById(paneId);
					if (target) target.classList.add('active', 'show');
					setFiltersVisible((categoryName || title) === SMILEYS_CATEGORY);
					applyFilters(wrapper);
					if (typeof conf.categoryclick === 'function') conf.categoryclick(e);
				});
				li.appendChild(a);
				menu.appendChild(li);
				return li;
			...
			function addPage(id, controlledBy, categoryName) {
				var div = document.createElement('div');
				div.className = qsAll(wrapper, '[role=tablist] > li').length > 1 ? 'tab-pane fade' : 'tab-pane fade active show';
				div.id = id;
				div.setAttribute('role', 'tabpanel');
				div.setAttribute('aria-labelledby', controlledBy);
				div.setAttribute('data-emoji-category', categoryName || '');
				list.appendChild(div);
				return div;
			}

			this.getCategories().forEach(function(cat, catIndex) {
				var nav = addNav(catIndex, cat.name, cat.name);
				var page = addPage(nav.firstChild.getAttribute('aria-controls'), nav.firstChild.id, cat.name);
				cat.getSubCategories().forEach(function(sub) {
					var group = document.createElement('div');
					group.className = 'emoji-list-group';
					page.appendChild(group);
					var dontAdd = [];
					sub.getEmojies().forEach(function(emoji) {
						if (emoji.code.split(' ').indexOf('FE0F') !== -1) dontAdd.push(emoji.name);
						var splitted = emoji.code.split(' ');
						var inKeyPadRange = splitted.indexOf('20E3') !== -1;
						var hasFE0F = splitted.indexOf('FE0F') !== -1;
						var hasCleanLastFE0F = hasFE0F && emoji.code.split('FE0F')[1] === '';
						if (dontAdd.indexOf(emoji.name) === -1 || (hasFE0F && hasCleanLastFE0F && !inKeyPadRange) || (inKeyPadRange && !hasFE0F)) {
							...
							['1F466', '1F467', '1F468', '1F469'].forEach(function(l) {
								if (emoji.code.indexOf(l) !== -1) b.setAttribute('emoji-family-type', '');
							});
							['2640', '2642'].forEach(function(l) {
								if (emoji.code.indexOf(l) !== -1 && emoji.code !== l && emoji.code !== l + ' FE0F') {
			...
			var firstCat = this.getCategories()[0];
			setFiltersVisible(firstCat && firstCat.name === SMILEYS_CATEGORY);
			applyFilters(wrapper);
			...
			if (conf.node) {
				try {
					var input = conf.node;
					var trigger = conf.trigger || null;
					if (!trigger) {
						// Only adjust layout when we inject our own emoji button
						if (input.parentNode && (input.parentNode.className || '').indexOf('input-group') === -1) {
							input.parentNode.className = (input.parentNode.className + ' input-group').trim();
						}
						var wrap = document.createElement('div');
						if (conf.prepend) {
							wrap.innerHTML = '<div class="input-group-prepend"><button type="button" class="btn btn-secondary btn-sm" aria-haspopup="true" aria-expanded="false">🙂</button></div>';
							trigger = wrap.firstChild.firstChild;
							trigger.onfocus = function() { input.focus(); };
							input.parentNode.insertBefore(wrap.firstChild, input);
						} else {
							wrap.innerHTML = '<div class="input-group-append"><button type="button" class="btn btn-secondary btn-sm" aria-haspopup="true" aria-expanded="false">🙂</button></div>';
							trigger = wrap.firstChild.firstChild;
							if (input.nextSibling) {
								input.parentNode.insertBefore(wrap.firstChild, input.nextSibling);
							} else {
								input.parentNode.appendChild(wrap.firstChild);
							}
						}
					}
					popoverApi = createPopover(trigger, wrapper, {
			...
			return { node: wrapper, attached: success, duration: Date.now() - start, popover: popoverApi };
		};
	}();

	Object.defineProperties(window, {
		Emoji: { configurable: false, writable: false, enumerable: true, value: Emoji },
		EmojiCategory: { configurable: false, writable: false, enumerable: true, value: EmojiCategory },
		EmojiSubCategory: { configurable: false, writable: false, enumerable: true, value: EmojiSubCategory },
		Emojilist: { configurable: false, writable: false, enumerable: true, value: Emojilist }
	});
}(window);
...
function initEmojiPicker() {
	...
	if (!btn || !input || typeof Emojilist === 'undefined') return;
	if (btn.getAttribute('data-emoji-ready') === '1') return;
	btn.setAttribute('data-emoji-ready', '1');

	var list = new Emojilist();
	list.attach({
		node: input,
		trigger: btn,
		prepend: false,
		...
		insert: function(emoji) {
			...
			// Ensure overlay preview refreshes even if listeners missed the event
			if (window.postManager && typeof window.postManager.schedulePreviewUpdate === 'function') {
				window.postManager.schedulePreviewUpdate();
			} else if (window.chatManager && window.chatManager.postManager && typeof window.chatManager.postManager.schedulePreviewUpdate === 'function') {
				window.chatManager.postManager.schedulePreviewUpdate();
			}
		},
